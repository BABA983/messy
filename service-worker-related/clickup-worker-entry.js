// The contents of this file are appended to the top of the service worker created by angular by the tools/ci/build-and-deploy/scripts/patch-build.ts script
// It's appended to the top so that it runs first, and if it doesn't need to intercept a request then the regular angular service worker will handle the request

// The patch to the service worker below will intercept any third party script (i.e. that is loaded outside of our codebase like google analytics etc)
// and unpatch the source to make sure all async operations (setTimeout, addEventListener etc) run outside the
// angular zone and consequently run less change detection cycles to improve performance
// We also do similar patching inside of our own code here: tools/webpack-config/patch-libraries.ts
// More info here on why runOutsideAngular isn't enough: https://click-up.slack.com/archives/C01E8ECSEDV/p1637337777088500
self.addEventListener('fetch', function (event) {
	// Only run on these hosts, if the url does not match then do nothing
	// and the regular angular service worker will handle the request.
	// Only add something here if you know it doesn't expose an api that will be used by angular
	// i.e. things like analytics tracking, widget code etc.
	// If the script exposes an async api that is used by an angular component then you can still add it here
	// but may need to manually trigger change detection if the result of the api call is used inside a template

	// this list was created by looking at the network tab for app.clickup.com and looking at all the 3rd party scripts that get loaded
	// we use the host instead of the full script patch because the script loaded from our app
	// will often load other scripts so we don't always know what the script urls will be

	// IMPORTANT: any hosts you add here _must_ allow CORS so the service worker can fetch and intercept them.
	// You can check this by opening app.clickup.com in chrome, open the console tab and then run:
	// await (await fetch('https://path/to/script.js')).text()
	// If you see the script contents then you know it should work ok. If there is no CORS then you'll see an error like this:
	// Access to fetch at 'https://path/to/script.js' from origin 'https://app.clickup.com' has been blocked by CORS policy: No 'Access-Control-Allow-Origin' header is present on the requested resource.

	// Even then that may not work, so it's worth pushing your changes to the fire branch for testing.
	// Wait for that to build and then open https://fire.clickup.com in chrome + safari, and check the browser console for any errors logged from the service worker

	/** Need to make an update to this file so the service worker re-registers */

	const { headers, url } = event.request;
	const unpatchHosts = [
	  'content.webproduct.clickup.com',
	  'cdn.pendo.io',
	  'cdn.servicebell.com',
	];
	...
	const hasSSERequest = false && headers.get('Accept') === 'text/event-stream';
	...
	if (hasSSERequest) {
	  ...
	  const onServerMessage = (
		controller,
		{ data, type, retry, lastEventId }
	  ) => {
		const responseText = sseChunkData(data, type, retry, lastEventId);
		const responseData = Uint8Array.from(responseText, (x) =>
		  x.charCodeAt(0)
		);
	...
	if (unpatchHosts.includes(host) && url.endsWith('.js')) {
	  event.respondWith(
		fetch(url).then((response) => {
		  // cannot use async functions here as this file isn't transpiled and therefore may break on older browsers
		  return response.text().then((body) => {
			// only patch javascript files, and ignore css and json etc
			// use startsWith because you get content types like application/javascript;charset=utf-8
			const contentTypeHeader = response.headers.get('content-type');
			if (
			  contentTypeHeader &&
			  contentTypeHeader.startsWith('application/javascript')
			) {
			  // console.log(`Unpatching ${url} from running inside zone.js`);

			  // Zone.js works by monkey patching async apis like setTimeout etc
			  // so it knows when an async operation occurred and will trigger change detection
			  // It makes the original api available under __zone_symbol__{originalName} and
			  // using this will _not_ trigger change detection. So we do a simple find and
			  // replace in the third party script to use the unpatched api that won't trigger change detection

			  // todo - see if there's a way to share this logic with tools/webpack-config/patch-libraries.ts
			  const globalsToUnpatch = [
				'setTimeout',
				'setInterval',
				'Promise',
				'addEventListener',
			  ...
			  globalsToUnpatch.forEach((name) => {
				// E.g. `__zone_symbol__setTimeout`, you can check it in DevTools by running
				// `window.__zone_symbol__setTimeout`.
				const nativeApiName = `__zone_symbol__${name}`;

				// `(?<!__)setTimeout` is a negative look-behind, this will ensure that the expression doesn't
				// start with `__`. We're doing this because we wanna replace only
				// `setTimeout` but not `__zone_symbol__setTimeout` (which is already replaced).
				// You can try this regexp by going to https://regex101.com and placing `(?<!__)setTimeout` into
				// "REGULAR EXPRESSION" input, then you can also place this into "TEST STRING":
				// setTimeout
				// __zone_symbol__setTimeout
				// You can see that it'll match only `setTimeout`.

				// We use a try catch here because not all browsers (latest safari and older chrome + FF) support negative lookbehind regexps
				// so if the users browser doesn't support negative lookbehinds we silence the error and they don't get the unpatched scripts
				try {
				  body = body.replace(
  ...
  // this patch is for when the user gets a push notification without the browser being open, and they click it to open the app
	...
	event.waitUntil(
	  clients
		...
		.then(function (clientList) {
		  Array.from(clientList).forEach((client) => {
  ...
  (() => {
	...
	var Adapter = class {
	  ...
	  newRequest(input, init) {
		if (
			/\.(svg|cur|jpg|png|webp|gif|ani|ico|jpeg)$/.test(input) || (
			  self.location.hostname !== 'app.clickup.com' && /\.(css|js|eot|otf|ttf|woff|woff2)$/.test(input)
			)
		  ) {
			input += ((input.indexOf('?') === -1 ? '?' : '&') + 'ngsw-domain=' + encodeURIComponent(self.location.hostname));
		  }
  ...
  ${msgIdle}`, { headers: this.adapter.newHeaders({ "Content-Type": "text/plain" }) });
	...
	var Driver = class {
	  ...
	  async fetchLatestManifest(ignoreOfflineError = false) {
		const res = await this.safeFetch(this.adapter.newRequest("https://app-cdn.clickup.com/ngsw.json?ngsw-cache-bust=" + Math.random()));
		if (!res.ok) {
		  if (res.status === 404) {
